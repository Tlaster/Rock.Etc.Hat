...
namespace Rocket.Chat.Net.Api
{
    public interface IInformation
    {
        ...
        /// </summary>
        /// <returns><see cref="ServerInfoResponse"/></returns>
        [Get("/api/info")]
        ...
        /// Requires Auth
        /// </summary>
        /// <param name="query">When type is users you can send an additional workspace field, that can be local (default) or all. workspace=all will work only if Federation is enabled.</param>
        /// <param name="offset">number of items to “skip” in the query, is zero based so it starts off at 0 being the first item.</param>
        /// <param name="count">the number of items to “get” in the query, is one based so to get only one you would pass in 1. If you want to get all of the records, then pass in 0 but this will only work if the setting (see below) allows it.</param>
        /// <returns></returns>
